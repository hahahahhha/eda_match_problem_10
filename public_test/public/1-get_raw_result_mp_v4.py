...
def run_test(group_id, filepath, txt_save_dir):
...
def run_tests(solution, group_id, timeout=60):
    ...
    for filename in file_list:
        ...
        if os.path.exists(txt_save_dir):
            print('[Warning]-----------------------------------------File exist!! Skip!!', filepath)
            continue
        
        tasks.append((group_id, filepath, txt_save_dir))

    # 使用多进程池执行任务
    with ProcessPoolExecutor(max_workers=max_processes) as executor:
        futures = {executor.submit(run_test, group_id, filepath, txt_save_dir): filename for group_id, filepath, txt_save_dir in tasks}
